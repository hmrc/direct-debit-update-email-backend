...
object AuthStub {
  ...
  def ensureAuthoriseCalled(authProviders: AuthProviders = AuthProviders(GovernmentGateway)) =
    verify(
      exactly(1),
  ...
  def ensureAuthoriseNotCalled() =
    verify(exactly(0), postRequestedFor(urlPathEqualTo(authoriseUrl)))
